 ...
 * Minimal DOM/router shim for static CI. No-ops unless `?shim=1` is present.
 *
 * Features:
 * - Accepts `?shim=1` or legacy `?shim=1/<path>` and normalizes to `/<path>?shim=1`.
 * - Renders deterministic markup with data-testids your Playwright tests expect.
 * - Simulates lap progression (4 biome completes => advance lap).
 * - Idempotent/robust bootstrap: runs even if DOM is already complete.
 */
(function () {
  // ---------- Activation ----------
  var hasShim =
  ...
  if (legacy && legacy[1]) {
    var fixed = '/' + legacy[1] + '?shim=1' + location.hash;
  ...
  // Expose tiny helpers for tests (safe no-ops outside CI)
  ...
  function Forest() {
    ...
    function completeOnce() {
      ...
      // Live counters for debug or DOM reads
  ...
  function Progress() {
    ...
    root.append(html`
      <section style="padding:16px;font-family:Inter,system-ui">
        <h2 data-testid="progress-heading">progress</h2>
      </section>
    `);
  }
  ...
  if (document.readyState === 'loading') {
    ...
    // DOM is already interactive/complete → render immediately
    boot();
  }

  // Extra safety & navigation
