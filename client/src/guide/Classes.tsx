...
export function Classes() {
  ...
  const body = (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      {!activeLearner && (
        <InlineError message="No active learner selected. Please select a learner in the roster tab." />
      )}
          ...
          <CardContent>
            ...
            <Toolbar left="">
              ...
              <Button 
                ...
                data-testid="button-save-new-class"
              >
                Add Class
              </Button>
            </Toolbar>
        ...
        <CardContent>
          {classes.length === 0 ? (
            <EmptyState 
              ...
              description="Create your first class to organize learners and manage projector settings."
              action={
              ...
              {classes.map((cls) => (
                <div key={cls.id} className={clsx(
                    ...
                    <div className="flex items-center gap-3">
                      {editingClass === cls.id ? (
                        <Input
                          ...
                          autoFocus
                          data-testid={`input-edit-class-name-${cls.id}`}
                        />
                      ) : (
                    ...
                    <div className="flex items-center gap-2">
                      {activeClassId !== cls.id && (
                        <Button 
                          size="sm" 
                          variant="outline"
                          onClick={() => handleSetActive(cls.id)}
                          data-testid={`button-set-active-${cls.id}`}
                        >
                      ...
                      {deleteConfirm === cls.id ? (
                        <div className="flex gap-1">
                          <Button 
                            ...
                            data-testid={`button-confirm-delete-${cls.id}`}
                          >
                            Confirm
                          </Button>
                          <Button 
                            ...
                            data-testid={`button-cancel-delete-${cls.id}`}
                          >
                            Cancel
                          </Button>
                        </div>
                  ...
                  <div className="text-sm text-gray-600 flex items-center gap-4">
                    ...
                    <span>Members: {cls.memberCount || 0}</span>
                  </div>
                </div>
              ))}
            </div>
      ...
      {/* Active class projector settings */}
      {activeClass && (
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
            ...
            <p className="text-sm text-gray-600">
              Configure display settings for <strong>{activeClass.name}</strong>
            </p>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <Field label="Theme">
                  <Select 
                    value={activeClass.projectorPreset?.theme || 'auto'}
                    onValueChange={(value) => updateProjectorPreset(activeClass.id, 'theme', value)}
                    data-testid={`select-theme-${activeClass.id}`}
                  >
                ...
                <Field label="Layout mode">
                  <Select 
                    value={activeClass.projectorPreset?.layoutMode || 'normal'}
                    onValueChange={(value) => updateProjectorPreset(activeClass.id, 'layoutMode', value)}
                    data-testid={`select-layout-mode-${activeClass.id}`}
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <label className="flex items-center gap-2">
                  <Switch
                    checked={activeClass.projectorPreset?.hideProgress || false}
                    onCheckedChange={(checked) => updateProjectorPreset(activeClass.id, 'hideProgress', checked)}
                    data-testid={`switch-hide-progress-${activeClass.id}`}
                  />
                  <span className="text-sm">Hide progress bars</span>
                </label>
