...
export function QuestIsland({ onLessonSelect }: QuestIslandProps) {
  ...
  const treeBranches = {
    ...
    lagoon: {
      trunkPoint: { x: 900, y: 612 }, // Right side on kidney trail, moved up by radius distance
      branches: [
        { 
          endpoint: { x: 840, y: 567 }, // Branch 1 - northwest spread from trail
          lessonId: "lagoon-1",
          angle: 135
        },
        { 
          endpoint: { x: 960, y: 662 }, // Branch 2 - southeast spread from trail
          lessonId: "lagoon-2",
          angle: 315
        },
        { 
          endpoint: { x: 840, y: 672 }, // Branch 3 - southwest spread from trail
          lessonId: "lagoon-3", 
          angle: 225
        }
      ]
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden">
        ...
        >
          <div className="relative w-72 h-72">
            ...
            <img 
              ...
              alt="Backpack Hover"
              className="absolute inset-0 w-full h-full object-contain drop-shadow-lg group-hover:drop-shadow-xl opacity-0 group-hover:opacity-100 scale-120 transition-all duration-300"
            />
          </div>
      ...
      {showScoutMessage && scoutMessage && (
        <motion.div
          className="fixed top-80 left-4 bg-white/95 text-gray-800 rounded-2xl shadow-lg backdrop-blur-sm border border-white/20 max-w-lg w-[22.4rem] z-50"
